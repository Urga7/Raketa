function Y = rocket_derivative(time, Y, has_fuel = false, tilting = false, tilt_speed = 0)
...
if has_fuel && tilting
  vel_derivative = (-(0.5 * base_air_density * (1 / exp((sqrt(pos_init(1)^2 + pos_init(2)^2) - earth_radius) / 8200)) * (sqrt(vel_init(1)^2 + vel_init(2)^2) * vel_init) * cross_section_area * drag_coeficient) / (dry_mass + fuel_init) ...
               ...
               + [fuel_consumption * fuel_ejection_speed * (1 / (dry_mass + fuel_init)) * sin(tilt_speed * time); fuel_consumption * fuel_ejection_speed * (1 / (dry_mass + fuel_init)) * cos(tilt_speed * time)]);
end
if has_fuel && !tilting
  vel_derivative = (-(0.5 * base_air_density * (1 / exp((sqrt(pos_init(1)^2 + pos_init(2)^2) - earth_radius) / 8200)) * (sqrt(vel_init(1)^2 + vel_init(2)^2) * vel_init) * cross_section_area * drag_coeficient) / (dry_mass + fuel_init) ...
               + [-((G * earth_mass) / sqrt(pos_init(1)^2 + pos_init(2)^2)^2) * (pos_init(1) / sqrt(pos_init(1)^2 + pos_init(2)^2)); -((G * earth_mass) / sqrt(pos_init(1)^2 + pos_init(2)^2)^2) * (pos_init(2) / sqrt(pos_init(1)^2 + pos_init(2)^2))] ...
               + [fuel_consumption * fuel_ejection_speed * (1 / (dry_mass + fuel_init)) * sin(0); fuel_consumption * fuel_ejection_speed * (1 / (dry_mass + fuel_init)) * cos(0)]);
end
if !has_fuel
  fuel_derivative = 0;
